...
pub fn dangerous_command_match(command: &[String]) -> Option<DangerousCommandMatch> {
    // Kodex fork: route shell commands that directly mutate files — and
    // scripted interpreters reading a stdin heredoc, whose body is not present
    // in the parsed argv — to the ACP host's permission broker. Otherwise the
